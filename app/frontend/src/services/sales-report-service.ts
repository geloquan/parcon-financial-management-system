import type { SalesReport, ApiCollectionResponse, SalesReportVersion } from '../types/api'
...
export const fetchPortfolioSalesReports = async (
  page = 1,
): Promise<ApiCollectionResponse<SalesReportVersion>> => {
  return apiRequest<ApiCollectionResponse<SalesReportVersion>>(
    `/portfolio/sales_reports?page=${page}`,
  )
}
...
export const downloadPortfolioSalesReport = async (
  reportId: number,
): Promise<{ blob: Blob; filename: string }> => {
  const token = getStoredToken()
  const response = await fetch(
    `${import.meta.env.VITE_API_BASE_URL ?? 'http://127.0.0.1:8000/api'}/portfolio/sales_reports/${reportId}/download`,
    {
      method: 'GET',
      headers: token ? { Authorization: `Bearer ${token}` } : undefined,
    },
